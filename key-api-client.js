...
(function () {
  ...
  function updateHomeKeyInfo() {
    ...
    try { renderExpiryWarning(); } catch (e) {}
  }
  ...
  async function fetchAnnounce() {
    try {
      ...
      renderContactAndSupport(data.settings);
      renderExpiryWarning();
    } catch (e) {}
  }

  function renderContactAndSupport(settings) {
    settings = settings || {};
    var id = "almContactBar";
    var el = document.getElementById(id);
    if (!el) {
      el = document.createElement("div");
      el.id = id;
      el.style.cssText = "display:none;margin:10px 12px 0;padding:12px;border-radius:14px;border:1px solid rgba(0,200,255,0.25);background:rgba(0,40,60,0.35);text-align:center;z-index:100002;position:relative";
      var gate = document.getElementById("keyGate");
      var box = gate && gate.querySelector(".key-box");
    ...
    var html = "";
    if (settings.supportNote) {
      html += '<div style="font-size:12px;line-height:1.5;color:#d8eef8;margin-bottom:8px;white-space:pre-wrap">' + escapeHtml(settings.supportNote) + '</div>';
    }
    if (settings.contactUrl) {
      var label = settings.contactText || "Liên hệ admin";
      html += '<a href="' + escapeHtml(settings.contactUrl) + '" target="_blank" rel="noopener" style="display:inline-block;padding:10px 16px;border-radius:12px;background:linear-gradient(135deg,rgba(0,200,255,0.25),rgba(59,130,246,0.2));border:1px solid rgba(0,200,255,0.4);color:#c8f0ff;font-weight:800;font-size:12px;text-decoration:none">' + escapeHtml(label) + '</a>';
    } else if (settings.contactText) {
      html += '<div style="font-size:12px;color:#a8d4e8;font-weight:600">' + escapeHtml(settings.contactText) + '</div>';
    }
    if (html) {
      el.innerHTML = html;
      el.style.display = "block";
    ...
    // Also bar in app
    var id2 = "almContactBarApp";
    var el2 = document.getElementById(id2);
    if (!el2) {
      el2 = document.createElement("div");
      el2.id = id2;
      el2.style.cssText = "display:none;margin:0 0 12px;padding:12px;border-radius:14px;border:1px solid rgba(0,200,255,0.25);background:rgba(0,40,60,0.35);text-align:center";
      var shell = document.querySelector(".app-shell");
      var header = shell && shell.querySelector(".app-header");
      if (header && header.parentNode) header.parentNode.insertBefore(el2, header.nextSibling);
    }
    if (html) {
      el2.innerHTML = html;
  ...
  function renderExpiryWarning() {
    var s = loadLocal();
    var id = "almExpiryWarn";
    var el = document.getElementById(id);
    if (!el) {
      el = document.createElement("div");
      el.id = id;
      el.style.cssText = "display:none;margin:0 0 12px;padding:12px 14px;border-radius:14px;border:1px solid rgba(255,200,80,0.4);background:linear-gradient(180deg,rgba(255,180,40,0.2),rgba(80,40,0,0.25));color:#ffe9b0;font-size:12.5px;font-weight:700;line-height:1.5;text-align:center";
      var shell = document.querySelector(".app-shell");
      var header = shell && shell.querySelector(".app-header");
      if (header && header.parentNode) header.parentNode.insertBefore(el, header.nextSibling);
      else if (shell) shell.insertBefore(el, shell.firstChild);
    }
    if (!s || !s.key || s.expiresAt == null) {
      el.style.display = "none";
      return;
    }
    ...
    if (left > 0 && left <= threeDays) {
      ...
      el.textContent = "⚠️ Key sắp hết hạn — còn khoảng " + days + " ngày. Liên hệ admin để gia hạn.";
      el.style.display = "block";
    } else if (left <= 0) {
      el.textContent = "⛔ Key đã hết hạn. Liên hệ admin để gia hạn.";
      el.style.display = "block";
    } else {
      el.style.display = "none";
    }
